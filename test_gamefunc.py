...
def fight(current_enemy):
    ...
    while player_hp > 0 and enemy_hp > 0:
        crit = randint(1, 100)
        if round % 2 == 1:
            print("Атакует ", player["name"])
            print("Вы нанесли ", player["attack"])
            print("Здоровье ", enemy["name"], "составляет ", enemy_hp)
            if crit <= player["luck"]:
            ...
            print("Атакует ", enemy["name"])
            player_hp -= enemy["attack"] * player["armor"]
            print("Ваше здоровье составляет ", player_hp, "единиц")
            sleep(1.5)
        round += 1
    if player_hp > 0:
        print(enemy["win"])
